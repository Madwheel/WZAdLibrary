...
public class SliderView extends RelativeLayout {
    ...
    public void bindData(List<String> jumpUrlList, List<String> imageUrlList, final List<Integer> imageMapIdList, final OnSlideShowListener listener) {
        ...
        if (imageUrlList.size() > 0) {
            ...
            if (!BaseToolsMain.getInstance().getControl().compareList(mImageUrlList, imageUrlList)) {
                mImageUrlList.clear();
    ...
    private void loadPoint(List<String> headerImages, LinearLayout ll_point_group) {
        ...
        // 设置每个小圆点距离左边的间距
        margin.setMargins(BaseToolsMain.getInstance().getControl().dip2px(mContext, 4)
                , BaseToolsMain.getInstance().getControl().dip2px(mContext, 3)
                , BaseToolsMain.getInstance().getControl().dip2px(mContext, 4), 0);
